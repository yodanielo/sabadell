...
{
 $this->ini_datos("sab_usuarios","usuario");
}
...
function ingresar()
{
	$this->datos->listaSP("select * from sab_usuarios where usuario='".str_replace("'","",trim($_POST["txt_user"]))."' and password='".md5(trim($_POST["txt_pass"]))."'","","");
...
if($_SESSION["user"]["password"]==trim(md5($_POST["password"]))) {
	$this->datos->ejecutar("update sab_usuarios set password='".md5(trim($_POST["nueva"]))."' where id=".$_SESSION["user"]["id"]);
...
{
$r = new MantixGrid();
$r->atributos=array("tabla"=>"sab_usuarios","nropag"=>"20","ordenar"=>"usuario");
$r->columnas=array(
